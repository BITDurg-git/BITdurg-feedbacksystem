<?php
	...
	if(isset($res->id))
	{
		...
		$topic = $crud->getWhere('topic','test_creator', "sid = '$id'");
		$uid = $_SESSION['userid'];
		$qid_set = $crud->getWhere('qid','response','student_response', "uid = '$uid' AND sid = '$id'");
...
<div class="w3-border w3-margin">
	<div class="w3-row w3-padding">
		...
		<div class="w3-col m4 w3-padding">
			<span>+<?=$a_data[0]['posmark']?>/-<?=$a_data[0]['negmark']?></span>
			<span>
				<?php
				if($q_data[0]['level'] != 0)
	...
	<div class="w3-border-top">
		...
		<div>
			<pre class="w3-border-top w3-padding">A)<?=htmlspecialchars($q_data[0]['a'])?></pre>
			<pre class="w3-border-top w3-padding">B)<?=htmlspecialchars($q_data[0]['b'])?></pre>
			<pre class="w3-border-top w3-padding">C)<?=htmlspecialchars($q_data[0]['c'])?></pre>
			<pre class="w3-border-top w3-border-bottom w3-padding">D)<?=htmlspecialchars($q_data[0]['d'])?></pre>
		</div>
...
<?php
		...
		echo "<h4 class='w3-center'>No record found</h4>";
?>