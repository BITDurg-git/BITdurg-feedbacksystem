<?php
	...
	for($i=0;$i<$N;$i++)
	{
		$attend_rs=mysqli_query($con,"select sum( distinct total_lectures ),sum(distinct present) from ".$batch_str." where student_id=$student_id[$i] and subject_id=$subject_id and (month_id between $min_month and ($month_id-1))") or die(include("faculty_exception.php"));
		
		$attend_rs1=mysqli_fetch_array($attend_rs);
		
		$outof[$i]-=$attend_rs1[0];
		echo $attend_rs1[1]."  ".$present[$i]."  "; 
	...
	for($i=0;$i<$N-1;$i++)
	{
		...
		mysqli_query($con,"insert into ".$batch_str."(month_id,subject_id,student_id,total_lectures,present) values".$str );
