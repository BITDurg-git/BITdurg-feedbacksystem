...
if($_SESSION['type'] == 'f')
{
	...
	switch ($goto)
	{
		...
		case 'getTest':
			...
			$data = $crud->getWhere('topic','sid','tnoq','uid', 'subject', "sem_sub_id = "."'".$sem_sub_id."'");
			echo json_encode($data);
			break;

		case 'createTest':
			$sem_sub_id = $_SESSION['sem_sub_id'];
			//$length = strlen($sem_sub_id);
			$data = $crud->getWhere('subject', "sem_sub_id = '$sem_sub_id'");
			$n = @count($data) + 1;
			$code = $sem_sub_id.'_'.$n;
			...
			$insert_into_subject = $crud->insert(
			...
			$insert_into_faculty = null;

			if($insert_into_subject == true)
			{
				$date = date("Y-m-d H:i:s");

				$insert_into_faculty = $crud->insert(
				'uid', "'".$_SESSION['userid']."'",
				'sid', "'$code'",
				'date_created', "'$date'",
				'faculty'
		...
		case 'dataEditTest':
			$sid = $_SESSION['sid'];
			$test_data = $crud->getWhere('subject',"sid = '$sid'");
		...
		case 'startTest':
			...
			$check = $crud->getWhere('is_live','online_test',"sid = '$sid' AND is_live = 1");
		...
		case 'liveTest':
			...
			$sid = $crud->getWhere('sid','online_test',"is_live = 1");
			if($sid == null)
			{
				echo json_encode(0);
				break;
			...
			{
				$m = $crud->getWhere('sem_sub_id','topic','subject',"sid = '".$value['sid']."'");
				$t[$i] = $m[0];
		...
		case 'studentList':
			$stud_list = $crud->getWhere('uid','name','user',"semester = ".intval($request->sem));
		...
		case 'reportByStudent':
			error_reporting(E_ERROR | E_PARSE);
			$data = $crud->getWhere('student_test_record', "LEFT(sid,1) = '".$request->sem."'");
			...
			{
				$sql_tm = "SELECT SUM(posmark) as TM FROM `answer_bank` NATURAL JOIN response where uid = '".$value['uid']."' and sid = '".$value['sid']."'";
				...
				$sub_top = "SELECT topic, subject_name FROM `subject` NATURAL JOIN sem_sub WHERE sid = '".$value['sid']."'";
