...
<div class='panel-heading' ><h3 class='panel-title' align="center" >Bhilai Institute Of Technology</h3></div>
    ...
    <?php
    if($radio_choice==1)
    echo"<font size='2'>"."ATTENDANCE REPORT : SEMESTER : ".$semester." / "." SECTION : ".$section." / "."BATCH-".$batch_class." / "."MONTH : IN ".$month." ".$year."</font>";
	else
		echo"<font size='2'>"."ATTENDANCE REPORT : SEMESTER : ".$semester." / "." SECTION : ".$section." / "."BATCH-".$batch_class." / "."MONTH : Upto ".$month." ".$year."</font>";
	?>
	</h3></div>
...
<div class='table-responsive' overflow='scroll'  >
    ...
    <table border="1" class="table" cellpadding="0">
...
if($radio_choice==1)
{
		...
		{		
				...
				//echo $batch_str;
				$outof_res=mysqli_query($con,"select max(total_lectures) from ".$batch_str." where subject_id=$temp_subject_id && month_id=$month_id and student_id in (select student_id from student_master where section='$section' and semester=$semester and class_batch=$batch_class)")or die("aayush");
		...
		echo"<tr style='background-color:#8fbc8f'>"."<th colspan='2' class='text-right' align=right>"."<font size='2' >"."TOTAL"."</font>"."</th>";
		$i=0;
		...
		echo"<th class='text-center' align=center>"."<font size='2'>".$sum_outof."</th>";
		echo"<th class='text-center' align=center>"."<font size='2'>"."100%"."</th>";
		echo"</tr>";
		
		
			
		
		$student_res=mysqli_query($con,"select student_name,class_roll,student_id from student_master where (semester=$semester && section='$section' && class_batch=$batch_class) order by class_roll")or die("aayush");
		...
		while($student_res1=mysqli_fetch_array($student_res))
		{
			...
			{	
				...
				echo"<td  class='text-center' align=center>"."<font size='2'>".$temp_class_roll."</font>"."</td>";
			...
			else
			{
				...
				for($yz=0;$yz<$xy;$yz++)
				{
					if($outof[$yz]>$total_lec[$yz])
				  	{	
						echo"<td  class='text-center' align=center>"."<font size='2'>".$present_array[$yz]."/".$total_lec[$yz]."</font>"."</td>";	
				  	}
				  	else
				  	{
						echo"<td  class='text-center' align=center>"."<font size='2'>".$present_array[$yz]."</font>"."</td>";	  
				  	}
				}
				
				
				
				echo"<td  class='text-center' align=center>"."<font size='2'>".$sum_present."</font>"."</td>";
			echo"<td  class='text-center' align=center>"."<font size='2'>".number_format((float)$perc,2, '.', '')."</font>"."</td>";
			echo"</tr>";
			}
...
else if($radio_choice==2)
{
	...
	$subject_names_res=mysqli_query($con,"select subject_name,subject_id from subject_master where semester=$semester  ");
		...
		{		
				...
				$temp_subject_id=$subject_names_res1[1];
					...
					while($month_set_res1=mysqli_fetch_array($month_set_res))
					{
						...
						$outof_res=mysqli_query($con,"select max(total_lectures) from ".$batch_str." where subject_id=$temp_subject_id && month_id=$temp_month and student_id in (select student_id from student_master where section='$section' and semester=$semester and class_batch=$batch_class)")or die("aayush");
						$outof_res1=mysqli_fetch_array($outof_res);
		...
		echo"<tr >"."<th></th>"."<th class='text-right' align=right>"."<font size='3'>"."TOTAL"."</font>"."</th>";
		$i=0;
		...
		echo"<th class='text-center' align=center>"."<font size='3'>".$sum_outof."</font>"."</th>";
		echo"<th class='text-center' align=center>"."<font size='3'>"."100%"."</font>"."</th>"."</tr>";
		
		
			
		
		$student_res=mysqli_query($con,"select student_name,class_roll,student_id from student_master where (semester=$semester && section='$section' && class_batch=$batch_class) order by class_roll");
		...
		else
		{
			...
			else
			{
				...
				{	
					for($yz=0;$yz<$mn;$yz++)
					{
						echo"<td class='text-center' align=center>"."<font size='2'>".$present_array2[$yz]."/".$total_array2[$yz]."</font>"."</td>";	
					}
				}
				else
				{
					for($yz=0;$yz<$mn;$yz++)
					{
						echo"<td  class='text-center' align=center>"."<font size='2'>".$present_array2[$yz]."</font>"."</td>";	
					}
				}
...
<td colspan="4"></td>
	<td colspan="6">
            			...
            			<div style="height:100px;">
            				...
            				<div><font size="2">Prof. Pawan Kumar Patnaik</font></div>
                                        <div><font size="2">Prof. Sargam Gupta</font></div>
            			</div>
                    </td>
                    <td colspan=4></td>
                     <td colspan="6">
